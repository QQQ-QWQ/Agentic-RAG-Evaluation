...
它不是正式知识库。正式实验前还需要补充多份学习资料、技术文档、表格数据和代码片段。
...
1. 建立最小正式知识库：收集 3-5 份资料，生成 `documents.csv` 和 `chunks.jsonl`。
2. 让 C0/C1 都从同一批 chunk 中检索，而不是只跑单文档临时索引。
3. 准备首批 20 条测试题和参考证据。
4. 跑 C0 vs C1 小规模对比。
5. 在 C1 稳定后系统化评测 C2：混合检索、rerank（`llm` / `none`）、邻块上下文与成本对比。
