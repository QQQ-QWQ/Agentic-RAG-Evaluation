...
一次索引构建后跑多个阶段；向量索引写入 Chroma（``ragkb_kb_*``，目录见 ``CHROMA_PERSIST_DIRECTORY``），
...
def build_c2_ablation_parser(*, add_help: bool = True) -> argparse.ArgumentParser:
...
def run_c2_ablation_from_args(args: argparse.Namespace) -> None:
...
def main(argv: list[str] | None = None) -> None:
    args = build_c2_ablation_parser().parse_args(argv)
    run_c2_ablation_from_args(args)
